...
def fo_gradient_2d(f, x, y):
    """Retourne grad(f)(X+omega) * p(omega) pour chaque échantillon."""
    X = np.array([x, y])
    omegas = rng.normal(0, sigma, size=(M, 2))
    Xs = X + omegas
    # gradient local évalué sur chaque point
    grads = grad_f2(Xs)
    # densité gaussienne de chaque omega
    p_vals = gaussian_density(omegas, sigma)
    # pondération par la densité
    grads_vals = grads * p_vals[:, None]
    return grads_vals, omegas, p_vals
