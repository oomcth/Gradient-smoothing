...
def grad_f2(x, y):
    """Gradient de f2(x,y) sauf sur x=0 ou y=0."""
    x = np.asarray(x)
    y = np.asarray(y)

    # df/dx : 1 si x>0, 0 si x<0
    dfdx = np.where(x > 0, 1.0, 0.0)
    dfdx = np.where(x == 0, np.nan, dfdx)  # indéfini sur x = 0 (optionnel)

    # df/dy : signe de y (+1 ou -1)
    dfdy = np.where(y > 0, 1.0, -1.0)
    dfdy = np.where(y == 0, np.nan, dfdy)  # indéfini sur y = 0 (optionnel)
