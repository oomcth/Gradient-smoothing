...
class F_contact(torch.autograd.Function):
    ...
    def forward(ctx, x1, x2, k, gtype=0):
        ctx.gtype = gtype
        ctx.k = k
        ctx.x1.detach().cpu().numpy()
        ctx.x2_np = x2.detach().cpu().numpy()
        f = F_contact_np(ctx.x1_np, ctx.x2_np, k)
        out = torch.from_numpy(f)
        return out[..., 0], out[..., 1]

    @staticmethod
    def backward(ctx, grad_output):
        if ctx.gtype == 0:
            gradx1, gradx2 = fo_gb(ctx.x1_np, ctx.x2_np, ctx.k)
        return (
            grad_output @ torch.from_numpy(gradx1),
            grad_output @ torch.from_numpy(gradx2),
            None,
        )
...
class PushingEnv(nn.Module):
    ...
    def step(self, x1, v1, x2, v2, u):
        F_contact_ = F_contact.apply(x1, x2, k)
