...
class MyFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, param):
        """
        ctx : contexte pour stocker des variables nécessaires au backward
        input, param : tenseurs d'entrée

        Retourne : sortie tensorielle
        """
        # Calcul direct, toutes opérations torch (ou même numpy si tu ne veux PAS autograd ici)
        output = input * param

        # Sauvegarde des éléments utiles pour le gradient
    ...
    def backward(ctx, grad_output):
        """
        ctx : contexte contenant ce qu'on a sauvegardé
        grad_output : gradient d’entrée venant de l’étape suivante
        """
        ...
        # Retourne les gradients dans le même ordre que les entrées du forward
