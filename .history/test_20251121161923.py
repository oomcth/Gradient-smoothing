...
def zo_gradient(f, x, sample_omega, grad_log_p, M=1000, rng=None):
    ...
    # evaluate f at x + omega
    xs = x.reshape(1, -1) + omegas  # (M, d)
    f_vals = np.array([f(xi) for xi in xs])  # (M,)
    # get grad_log_p for each omega
    glp = grad_log_p(omegas)  # (M, d)
    integrand = - (f_vals[:, None] * glp)  # (M, d)
