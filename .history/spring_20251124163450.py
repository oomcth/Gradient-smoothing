...
for gtype, name in zip(gtypes, names):
    ...
    print(name, "moyenne:", grads.mean(), "variance:", grads.var())
...
plt.bar(x - 0.2, means, width=0.4, label="moyenne")
plt.bar(x + 0.2, vars_, width=0.4, label="variance")
plt.xticks(x, names)
plt.legend()
plt.title("Statistiques des gradients v0")
plt.show()

env = PushingEnv(H=200, k=100, c=1)
u_seq = torch.full((env.H,), 1.0, requires_grad=True)
x1_0 = torch.tensor(0.0)
v1_0 = torch.tensor(10.0)
x2_0 = torch.tensor(2.0)
v2_0 = torch.tensor(0.0)
...
traj_x1, traj_x2 = env.step(x1_0, v1_0, x2_0, v2_0, 1.0, 2), [], []
